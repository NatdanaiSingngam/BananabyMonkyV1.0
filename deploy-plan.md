...
npx wrangler d1 create banana-by-monky-db
...
2. ตั้งชื่อ เช่น `banana-by-monky-deploy`
...
| 5 | `PAGES_PROJECT_NAME` | ชื่อ project ตั้งเอง ต้อง **unique ทั่วโลก** เช่น `banana-by-monky` | ตั้งเอง |
...
`https://banana-by-monky-backend.<subdomain>.workers.dev`
...
gh secret set VITE_BACKEND_URL --body "https://banana-by-monky-backend.<subdomain>.workers.dev"
```
...
curl https://banana-by-monky-backend.<subdomain>.workers.dev/health

# API docs (เปิดในเบราว์เซอร์)
# https://banana-by-monky-backend.<subdomain>.workers.dev/docs
...
npx wrangler d1 migrations apply banana-by-monky-db --remote
npm run deploy

# --- Frontend ---
cd ../frontend
pnpm install
VITE_BACKEND_URL="https://banana-by-monky-backend.<subdomain>.workers.dev" pnpm build
